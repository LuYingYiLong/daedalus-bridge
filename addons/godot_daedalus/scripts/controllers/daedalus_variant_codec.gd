...
static func encode(value: Variant, depth: int = 0, max_depth: int = 4, max_items: int = 500) -> Variant:
	...
	match value_type:
		...
		TYPE_ARRAY:
			...
			var array_limit: int = mini(source_array.size(), mini(max_items, MAX_COLLECTION_ITEMS))
			var encoded_array: Array = []
			for index: int in range(array_limit):
				encoded_array.append(encode(source_array[index], depth + 1, max_depth, max_items))
			if source_array.size() > array_limit:
				encoded_array.append({ "$type": "Truncated", "remaining": source_array.size() - array_limit })
		...
		TYPE_DICTIONARY:
			...
			for index: int in range(dictionary_limit):
				...
				entries.append({
					...
					"value": encode(source_dictionary[key], depth + 1, max_depth, max_items)
				})
			return {
				"$type": "Dictionary",
				"entries": entries,
		...
		TYPE_PACKED_VECTOR4_ARRAY:
			return _tag("PackedVector4Array", _encode_packed_values(Array(value), depth, max_depth, max_items))
		TYPE_RID:
		...
		_:
			return { "$type": "Opaque", "godotType": type_string(value_type), "value": str(value), "writable": false }
...
static func _decode(payload: Variant, expected: Variant, depth: int) -> Dictionary:
	...
	if tag.is_empty():
		...
		for key: Variant in source.keys():
			...
			plain_dictionary[str(key)] = decoded_value.get("value")
		return _ok(plain_dictionary)
	...
	if tag == "Dictionary":
		...
		for entry_value: Variant in entries_value as Array:
			...
			if not bool(decoded_value.get("ok", false)):
				return decoded_value
			dictionary[decoded_key.get("value")] = decoded_value.get("value")
	...
	match tag:
		...
		"PackedVector4Array":
			return _decode_packed(values, depth, TYPE_VECTOR4)
		_:
...
static func _encode_packed_values(values: Array, depth: int, max_depth: int, max_items: int) -> Array:
	var result: Array = []
	for index: int in range(mini(values.size(), max_items)):
...
static func _decode_packed(values: Array, depth: int, expected_type: int) -> Dictionary:
	...
	match expected_type:
		...
		TYPE_VECTOR4:
			return _ok(PackedVector4Array(decoded_values))
