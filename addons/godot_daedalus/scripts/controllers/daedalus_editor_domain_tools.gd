@tool
class_name DaedalusEditorDomainTools
extends RefCounted
...
func _search_classes(args: Dictionary) -> Dictionary:
	...
	var matched: Array[StringName] = []
	for class_name_value: StringName in ClassDB.get_class_list():
	...
	for index: int in range(offset, mini(offset + limit, matched.size())):
...
func _get_class_schema(args: Dictionary) -> Dictionary:
	...
	var schema: Dictionary = {
		...
		"parent": ClassDB.get_parent_class(class_name_text),
		"canInstantiate": ClassDB.can_instantiate(class_name_text),
		"apiType": ClassDB.class_get_api_type(class_name_text),
...
func _compact_class_members(members: Array[Dictionary], limit: int) -> Array[Dictionary]:
	var compact: Array[Dictionary] = []
	for index: int in range(mini(members.size(), limit)):
		var member: Dictionary = members[index]
		var item: Dictionary = {}
		for key: String in ["name", "type", "hint", "hint_string", "usage", "flags", "return"]:
			if member.has(key):
...
func _serialize_object(target: Object, offset: int = 0, limit: int = 100, max_depth: int = 2) -> Dictionary:
	...
	var visible_properties: Array[Dictionary] = []
	...
	for index: int in range(offset, mini(offset + limit, visible_properties.size())):
		...
		properties.append({
			...
			"typeName": type_string(int(property_info.get("type", TYPE_NIL))),
			"hint": property_hint,
...
func _resolve_resource_target(args: Dictionary, allow_plugin_read: bool) -> Dictionary:
	...
	for path_part: Variant in property_path_value as Array:
...
func _apply_resource_patch(args: Dictionary) -> Dictionary:
	...
	for prepared_operation: Dictionary in prepared.get("operations", []) as Array:
...
func _inspect_animation(args: Dictionary, edited_root: Node) -> Dictionary:
	...
	if target is AnimationMixer:
		var libraries: Array[Dictionary] = []
		for library_name: StringName in (target as AnimationMixer).get_animation_library_list():
			var library: AnimationLibrary = (target as AnimationMixer).get_animation_library(library_name)
		...
		return {
			"ok": true,
			"kind": "AnimationMixer",
			"class": (target as AnimationMixer).get_class(),
			"libraries": libraries,
...
func _apply_animation_patch(args: Dictionary, edited_root: Node) -> Dictionary:
	...
	if str(prepared.get("kind", "")) == "mixer":
		var mixer: AnimationMixer = prepared.get("mixer") as AnimationMixer
...
func _prepare_animation_patch(args: Dictionary, edited_root: Node) -> Dictionary:
	...
	if not target is AnimationMixer:
		return { "ok": false, "error": "animation_target_must_be_animation_mixer_or_tree" }
	var before_libraries: Dictionary = _duplicate_animation_libraries(target as AnimationMixer)
...
func _duplicate_animation_libraries(mixer: AnimationMixer) -> Dictionary:
	var libraries: Dictionary = {}
	for library_name: StringName in mixer.get_animation_library_list():
		libraries[str(library_name)] = mixer.get_animation_library(library_name).duplicate(true)
	return libraries
...
func _summarize_animation_libraries(libraries: Dictionary) -> Dictionary:
	...
	for key: Variant in libraries.keys():
...
func _serialize_animation_node(node: AnimationNode, depth: int = 0) -> Dictionary:
	...
	if node is AnimationNodeBlendTree:
		...
		for child_name: StringName in (node as AnimationNodeBlendTree).get_node_list():
			children.append({
				"name": str(child_name),
		...
		for state_name: StringName in (node as AnimationNodeStateMachine).get_node_list():
			states.append({
				"name": str(state_name),
		...
		for transition_index: int in range((node as AnimationNodeStateMachine).get_transition_count()):
			transitions.append({
		...
		var points_1d: Array[Dictionary] = []
		for point_index: int in range((node as AnimationNodeBlendSpace1D).get_blend_point_count()):
			points_1d.append({
				"index": point_index,
		...
		var points_2d: Array[Dictionary] = []
		for point_index: int in range((node as AnimationNodeBlendSpace2D).get_blend_point_count()):
			points_2d.append({
				"index": point_index,
		...
		var triangles: Array = []
		for triangle_index: int in range((node as AnimationNodeBlendSpace2D).get_triangle_count()):
			triangles.append([
...
func _replace_animation_libraries(mixer: AnimationMixer, libraries: Dictionary) -> void:
	for library_name: StringName in mixer.get_animation_library_list():
		mixer.remove_animation_library(library_name)
	for library_key: Variant in libraries.keys():
		mixer.add_animation_library(StringName(str(library_key)), libraries[library_key] as AnimationLibrary)
...
func _inspect_map(args: Dictionary, edited_root: Node) -> Dictionary:
	...
	if target is TileMapLayer:
		for cell: Vector2i in (target as TileMapLayer).get_used_cells():
			if not _cell_in_region_2d(cell, region):
				continue
			cells.append({
				"position": VARIANT_CODEC.encode(cell),
				"sourceId": (target as TileMapLayer).get_cell_source_id(cell),
				"atlasCoords": VARIANT_CODEC.encode((target as TileMapLayer).get_cell_atlas_coords(cell)),
				"alternativeTile": (target as TileMapLayer).get_cell_alternative_tile(cell)
			})
			if cells.size() >= limit:
				break
		return {
			...
			"tileSet": VARIANT_CODEC.encode((target as TileMapLayer).tile_set),
			"cells": cells,
			"truncated": cells.size() >= limit,
			"fingerprint": VARIANT_CODEC.fingerprint(cells)
		}
	if target is GridMap:
		for cell: Vector3i in (target as GridMap).get_used_cells():
...
func _prepare_map_patch(args: Dictionary, edited_root: Node) -> Dictionary:
	...
	if not bool(resolved.get("ok", false)):
		return resolved
	var target: Node = resolved.get("node") as Node
	if not target is TileMapLayer and not target is GridMap:
		return { "ok": false, "error": "node_is_not_supported_map" }
	var operations_value: Variant = args.get("operations", [])
	if typeof(operations_value) != TYPE_ARRAY:
		return { "ok": false, "error": "invalid_operations" }
	...
	if operations.is_empty() or operations.size() > MAX_PATCH_OPERATIONS:
		return { "ok": false, "error": "invalid_operation_count" }
	var before: Array[Dictionary] = _snapshot_map_cells(target)
	var after: Array[Dictionary] = before.duplicate(true)
	var before_tile_set: TileSet = null
	var after_tile_set: TileSet = null
	if target is TileMapLayer:
		before_tile_set = (target as TileMapLayer).tile_set
	...
	if target is TileMapLayer and after_tile_set != null:
		for cell: Dictionary in after:
...
func _snapshot_map_cells(target: Node) -> Array[Dictionary]:
	var cells: Array[Dictionary] = []
	if target is TileMapLayer:
		for position: Vector2i in (target as TileMapLayer).get_used_cells():
			cells.append({
				"position": position,
				"sourceId": (target as TileMapLayer).get_cell_source_id(position),
				"atlasCoords": (target as TileMapLayer).get_cell_atlas_coords(position),
				"alternativeTile": (target as TileMapLayer).get_cell_alternative_tile(position)
			})
	elif target is GridMap:
		for position: Vector3i in (target as GridMap).get_used_cells():
			cells.append({
				"position": position,
...
func _apply_virtual_map_operation(cells: Array[Dictionary], target: Node, operation: Dictionary) -> String:
	var operation_type: String = str(operation.get("type", ""))
	if operation_type in ["clear", "clear_cells", "replace_all"]:
		...
		for replacement: Variant in replacement_value as Array:
...
func _restore_map_state(target: Node, cells: Array, tile_set: TileSet) -> void:
	if target is TileMapLayer:
		(target as TileMapLayer).tile_set = tile_set
		(target as TileMapLayer).clear()
		for cell_value: Variant in cells:
			var cell: Dictionary = cell_value as Dictionary
			(target as TileMapLayer).set_cell(
...
func _decode_map_position(payload: Variant, target: Node) -> Dictionary:
	var expected: Variant = Vector2i.ZERO if target is TileMapLayer else Vector3i.ZERO
	var decoded: Dictionary = VARIANT_CODEC.decode(payload, expected)
	if not bool(decoded.get("ok", false)):
		return decoded
	if target is TileMapLayer and not decoded.get("value") is Vector2i:
...
func _find_map_cell(cells: Array[Dictionary], position: Variant) -> int:
...
func _propose_audio_patch(args: Dictionary) -> Dictionary:
	...
	if not bool(validated.get("ok", false)):
		return validated
	var before: Array[Dictionary] = _summarize_audio_buses()
	return {
		"ok": true,
		"valid": true,
...
func _apply_audio_patch(args: Dictionary) -> Dictionary:
	...
	if not bool(validated.get("ok", false)):
		return validated
	var before_summary: Array[Dictionary] = _summarize_audio_buses()
	...
	var after_summary: Array[Dictionary] = _summarize_audio_buses()
	AudioServer.set_bus_layout(before_layout)
...
func _simulate_audio_operations(before: Array[Dictionary], operations: Array) -> Dictionary:
	var buses: Array[Dictionary] = before.duplicate(true)
	for operation_value: Variant in operations:
		var operation: Dictionary = operation_value as Dictionary
		var operation_type: String = str(operation.get("type", ""))
...
func _find_virtual_audio_bus(buses: Array[Dictionary], bus_name: String) -> int:
...
func _summarize_audio_buses() -> Array[Dictionary]:
	var buses: Array[Dictionary] = []
	for bus_index: int in range(AudioServer.bus_count):
		var effects: Array[Dictionary] = []
		for effect_index: int in range(AudioServer.get_bus_effect_count(bus_index)):
...
func _get_performance_snapshot(args: Dictionary) -> Dictionary:
	...
	for requested_name_value: Variant in requested:
...
func _navigate(args: Dictionary, edited_root: Node) -> Dictionary:
	...
	if action == "select_nodes":
		...
		for node_path_value: Variant in node_paths_value as Array:
...
func _reimport_assets(args: Dictionary) -> Dictionary:
	...
	for path_value: Variant in paths_value as Array:
