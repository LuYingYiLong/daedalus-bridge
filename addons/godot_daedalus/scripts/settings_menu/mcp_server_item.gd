...
func setup(metadata: Dictionary) -> void:
	...
	var description: String = str(metadata.get("description", "")).strip_edges()

	name_label.text = server_name
	details_label.text = description if not description.is_empty() else _format_connection_details(metadata)
	details_label.tooltip_text = _format_tooltip(metadata)
	status_label.text = "%s  ·  %s  ·  %d %s" % [
		_format_status(status),
		transport if not transport.is_empty() else "MCP",
		tool_count,
		"tool" if tool_count == 1 else "tools"
	]
	status_label.tooltip_text = _format_tooltip(metadata)
...
func _format_connection_details(metadata: Dictionary) -> String:
	...
	if not url.is_empty():
		return url
	return "Custom Model Context Protocol server"
...
func _format_tooltip(metadata: Dictionary) -> String:
	...
	if not env_names.is_empty():
		lines.append("Environment: %s" % ", ".join(_string_array_from_array(env_names)))
