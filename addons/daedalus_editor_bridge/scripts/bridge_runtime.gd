...
const BACKEND_RUNTIME_PATH: String = "res://addons/daedalus_editor_bridge/scripts/backend_runtime.gd"
const BRIDGE_CONNECTION_PATH: String = "res://addons/daedalus_editor_bridge/scripts/bridge_connection.gd"
const EDITOR_CONTEXT_PATH: String = "res://addons/daedalus_editor_bridge/scripts/editor_context.gd"
const STATUS_DOCK_SCENE_PATH: String = "res://addons/daedalus_editor_bridge/scenes/bridge_status_dock.tscn"

var editor_plugin: EditorPlugin
var backend_runtime: RefCounted
var connection: Node
var editor_context: Node
var status_dock: VBoxContainer
var status_value: Label
var project_value: Label
var scene_value: Label
var version_value: Label
var sync_value: Label
var capability_value: Label
var error_value: Label
...
func setup(plugin: EditorPlugin) -> void:
	...
	if backend_runtime_script == null or bridge_connection_script == null or editor_context_script == null:
		_set_error("One or more Editor Bridge runtime scripts could not be loaded.")
		return
...
func _on_connected() -> void:
	reconnect_attempt = 0
	_set_status("Negotiating Bridge Protocol v4")
...
func _build_status_dock() -> bool:
	...
	if dock_scene == null:
		push_error("Daedalus Editor Bridge status Dock scene could not be loaded.")
		return false
	status_dock = dock_scene.instantiate() as VBoxContainer
	if status_dock == null:
		push_error("Daedalus Editor Bridge status Dock scene has an invalid root node.")
		return false
	status_value = status_dock.get_node("StatusRow/Value") as Label
	project_value = status_dock.get_node("ProjectRow/Value") as Label
	scene_value = status_dock.get_node("SceneRow/Value") as Label
	version_value = status_dock.get_node("VersionRow/Value") as Label
	sync_value = status_dock.get_node("SyncRow/Value") as Label
	capability_value = status_dock.get_node("CapabilityRow/Value") as Label
	error_value = status_dock.get_node("ErrorRow/Value") as Label
	var reconnect_button: Button = status_dock.get_node("PrimaryActions/ReconnectButton") as Button
	var studio_button: Button = status_dock.get_node("PrimaryActions/OpenStudioButton") as Button
	var diagnostics_button: Button = status_dock.get_node("CopyDiagnosticsButton") as Button
	reconnect_button.connect(&"pressed", Callable(self, "_on_reconnect_pressed"))
	studio_button.connect(&"pressed", Callable(self, "_on_open_studio_pressed"))
	diagnostics_button.connect(&"pressed", Callable(self, "_on_copy_diagnostics_pressed"))
	project_value.text = ProjectSettings.globalize_path("res://").trim_suffix("/")
	version_value.text = "Bridge %s · Godot %s · Backend unknown" % [BRIDGE_VERSION, str(Engine.get_version_info().get("string", ""))]
	_set_status("Starting")
	return true


func _update_context_labels(context: Dictionary) -> void:
	scene_value.text = str(context.get("activeScenePath", "No open scene"))
	sync_value.text = Time.get_time_string_from_system()
	var capabilities_value: Variant = context.get("capabilities", {})
	var capabilities: Dictionary = capabilities_value as Dictionary if typeof(capabilities_value) == TYPE_DICTIONARY else {}
	var enabled_count: int
	for value in capabilities.values():
		if value == true:
			enabled_count += 1
	capability_value.text = "%d available" % enabled_count
	version_value.text = "Bridge %s · Godot %s · Backend %s" % [BRIDGE_VERSION, str(Engine.get_version_info().get("string", "")), backend_version]
...
func _on_copy_diagnostics_pressed() -> void:
	DisplayServer.clipboard_set("\n".join(PackedStringArray([
		"Daedalus Editor Bridge %s" % BRIDGE_VERSION,
		...
		"Status: %s" % status_value.text,
		"Context revision: %d" % context_revision,
		"Last error: %s" % error_value.text,
	])))
