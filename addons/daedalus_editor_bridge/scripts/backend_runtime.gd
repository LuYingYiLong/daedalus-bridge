...
var lease_id: String
var managed_executable_path: String
...
func start_backend() -> Dictionary:
	...
	if executable_path.is_empty():
		...
		return _failure_result()
	managed_executable_path = executable_path
	...
	lease_id = str(result.get("leaseId", "")).strip_edges()
	return {
		"ok": true,
		"mode": launch_mode,
...
func stop_started_backend() -> Dictionary:
	if lease_id.is_empty() or managed_executable_path.is_empty():
		return { "ok": true, "released": false }
	var output_lines: Array = []
	var exit_code: int = OS.execute(
		managed_executable_path,
		PackedStringArray(["runtime", "release", "--lease", lease_id, "--json"]),
		output_lines,
		true
	)
	var released_lease_id: String = lease_id
	lease_id = ""
	if exit_code != 0:
		return {
			"ok": false,
			"released": false,
			"leaseId": released_lease_id,
			"message": "Shared backend lease release failed.",
		}
	return {
		"ok": true,
		"released": true,
		"leaseId": released_lease_id,
		"message": "The Bridge lease was released; the shared backend remains available to other clients.",
	}
...
func _supports_shared_runtime(managed_backend: Dictionary) -> bool:
	...
	if minimum_protocol <= 0 or maximum_protocol < minimum_protocol:
		...
		return false
	if BRIDGE_PROTOCOL_VERSION < minimum_protocol or BRIDGE_PROTOCOL_VERSION > maximum_protocol:
		last_error = "Managed backend %s supports Bridge Protocol %d-%d, but this Bridge requires protocol %d. Update Daedalus Studio, Backend, and Editor Bridge together." % [backend_version, minimum_protocol, maximum_protocol, BRIDGE_PROTOCOL_VERSION]
		return false
	return true
...
func _stringify_output_lines(output_lines: Array) -> PackedStringArray:
	...
	for line_value in output_lines:
