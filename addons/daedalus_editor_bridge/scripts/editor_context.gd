...
const EDITOR_DOMAIN_TOOLS_PATH: String = "res://addons/daedalus_editor_bridge/scripts/editor_domain_tools.gd"
const VARIANT_CODEC_PATH: String = "res://addons/daedalus_editor_bridge/scripts/variant_codec.gd"
...
func get_capabilities() -> Dictionary:
	var can_capture_scene: bool = editor_interface != null \
		and editor_interface.has_method("get_editor_viewport_2d") \
		and editor_interface.has_method("get_editor_viewport_3d")
	return {
		"editorTools": true,
		"editorUndoRedo": editor_undo_redo != null,
		"sceneViewCapture": can_capture_scene,
		"typedVariantV1": true,
		"scenePatchV2": editor_undo_redo != null,
		"resourcePatchV1": editor_undo_redo != null,
		"animationPatchV1": editor_undo_redo != null,
		"mapPatchV1": editor_undo_redo != null,
		"audioPatchV1": editor_undo_redo != null,
		"editorNavigationV1": editor_interface != null,
		"safePreviewV1": true,
	}
...
func setup(plugin: EditorPlugin) -> void:
	...
	var editor_domain_tools_script: GDScript = load(EDITOR_DOMAIN_TOOLS_PATH) as GDScript
	var variant_codec_script: GDScript = load(VARIANT_CODEC_PATH) as GDScript
	if editor_domain_tools_script == null or variant_codec_script == null:
		push_error("Editor Bridge context dependencies could not be loaded.")
		return
	editor_domain_tools = editor_domain_tools_script.new()
...
func _sync_live_editor_selection_context(edited_root: Node, selected_nodes: Array) -> void:
	...
	for selected_node_info in selected_nodes:
...
func collect_filesystem_selection_context() -> Dictionary:
	...
	for index in range(selected_paths.size()):
...
func _compact_variant_for_json(value: Variant) -> Variant:
	...
	if typeof(value) == TYPE_DICTIONARY:
		...
		var compact_map: Dictionary = {}
		for key_value in source_map.keys():
			compact_map[str(key_value)] = _compact_variant_for_json(source_map[key_value])
		return compact_map
	return value
...
func handle_tool_requested(data: Dictionary) -> void:
	...
	if tool_name == "get_context":
		...
		ok = false
		error_message = "unsupported_editor_tool: Unknown editor tool: %s" % tool_name
...
func _emit_editor_tool_result(call_id: String, ok: bool, result: Variant, error_message: String) -> void:
	var failure: Variant = null
	if not ok:
		failure = _create_structured_failure(error_message)
	request_ready.emit(
		"editor.tool.result",
		{
			"callId": call_id,
...
func _create_structured_failure(error_message: String) -> Dictionary:
	var normalized: String = error_message.strip_edges()
	var separator_index: int = normalized.find(":")
	var code: String = "editor_tool_failed"
	var message: String = normalized
	if normalized.is_valid_identifier():
		code = normalized
	if separator_index > 0:
		var candidate: String = normalized.substr(0, separator_index).strip_edges()
		if candidate.is_valid_identifier():
			code = candidate
			message = normalized.substr(separator_index + 1).strip_edges()
	if message.is_empty():
		message = "The editor tool did not complete."
	return {
		"code": code,
		"message": message,
		"retryable": code not in ["unsupported_editor_tool", "unsupported_capability"],
	}
...
func _execute_editor_capture_scene_view(args: Dictionary) -> Dictionary:
	...
	if not bool(get_capabilities().get("sceneViewCapture", false)):
		return { "ok": false, "error": "unsupported_capability: Scene viewport capture is unavailable in this Godot version." }
	...
	var viewport_2d: SubViewport = editor_interface.call("get_editor_viewport_2d") as SubViewport
	...
	if selected_view == "3d":
		for viewport_index in range(4):
			var viewport_3d: SubViewport = editor_interface.call("get_editor_viewport_3d", viewport_index) as SubViewport
...
func _execute_editor_refresh_filesystem(args: Dictionary) -> Dictionary:
	...
	if typeof(changed_paths_value) == TYPE_ARRAY:
		for path_value in changed_paths_value as Array:
...
func _add_editor_patch_operation(operation: Dictionary, edited_root: Node, created_nodes: Array) -> String:
...
func _add_editor_add_node_operation(operation: Dictionary, edited_root: Node, created_nodes: Array) -> String:
	var parent_node: Node = _find_editor_node("", str(operation.get("parentPath", ".")))
	var node_type: String = str(operation.get("nodeType", "Node"))
...
func _add_editor_duplicate_node_operation(operation: Dictionary, edited_root: Node, created_nodes: Array) -> String:
...
func _add_editor_instantiate_scene_operation(operation: Dictionary, edited_root: Node, created_nodes: Array) -> String:
